...
async function resolveSrc(
  ...
  switch (type) {
    ...
    case "embed":
      return src;
    case "flash":
      return `/compat/flash#${encryptURL(src)}`;
    case "emulator":
    case "emulator.gba":
    case "emulator.nes":
    case "emulator.n64":
...
const TheatrePlayer = ({ data }: { data: TheatreEntry }) => {
    ...
    (async function () {
      ...
      try {
        ...
        const resolvedSrc = await resolveSrc(
          new URL(data.src, location.origin + "/cdn/").toString(),
          data.type
        );
        errorCause = undefined;
  ...
  // iframe focus event handlers
  useEffect(() => {
    if (!iframe.current || !iframeFocused) return;

    document.documentElement.scrollTo(0, 0);
    document.documentElement.setAttribute("data-lock-frame-scroll", "");
    ...
    return () => {
      ...
      document.documentElement.removeAttribute("data-lock-frame-scroll");
    };
      ...
      switch (key) {
        ...
        default:
          visuals.push(
            <div
              key={key}
              className={`${styles.controlKey} ${styles[`key${key}`]}`}
            >
              {key}
            </div>
          );
  ...
  return (
    <main
      ...
      data-controls={controlsExpanded || undefined}
    >
